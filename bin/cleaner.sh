...
found=()
while read -r pkg; do
  if LC_ALL=C pacman -Qi $pkg | tail -6 | head -1 | grep -qE 'Jun 9|Jun 10|Jun 11|Jun 12|Jun 13|Jun 14'; then
    found+=("$pkg")
  fi
done < <(pacman -Qq "${INFECTED_PKGS[@]}" 2>/dev/null)

if [[ ${#found[@]} -eq 0 ]]; then
  echo "Clean: None of the known infected packages were installed within 2 days of the campaigns."
else
